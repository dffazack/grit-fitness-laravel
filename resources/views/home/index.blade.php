...
/* === Perbaikan responsive untuk Popular Packages (mobile friendly) === */

/* Pastikan card menggunakan box-sizing dan penuh lebar kontainer kolom */
.membership-card {
  width: 100%;
  ...
  min-height: clamp(320px, 30vw, 560px); /* tetap responsif; ubah jika perlu */
}

/* Body kartu agar mengisi vertikal dan tombol mt-auto bekerja */
.membership-card .card-body,
.membership-card .card-body-grid {
  display: flex;
  flex-direction: column;
  height: 100%;
  padding: 1.25rem;
...
/* List fitur ambil sisa ruang dan scroll bila konten panjang */
.membership-card .list-unstyled {
  flex: 1 1 auto;
  overflow-y: auto;
  margin-bottom: 1rem;
...
/* Adjustments khusus untuk layar kecil supaya kartu tampak lebih 'full' */
@media (max-width: 575.98px) {
  /* kurangi efek gutter berlebihan pada kolom mobile (tetap beri sedikit padding) */
  .row.g-4 > [class*="col-"] {
    padding-left: 0.6rem;
    padding-right: 0.6rem;
  }

  /* pastikan card memenuhi kolom tanpa offset */
  .membership-card {
    min-height: auto;    /* mobile: biarkan tinggi menyesuaikan konten */
    width: 100%;
    border-radius: 12px;
  }
  ...
  /* kalau mau list fitur tidak membuat halaman makin panjang, batasi sedikit */
  .membership-card .list-unstyled {
    max-height: 320px;
  }
}

/* Opsi full-bleed (jika kamu ingin kartu benar-benar melebar ke tepi layar mobile)
   - Gunakan salah satu: aktifkan dengan menambahkan kelas .full-bleed pada section
*/
@media (max-width: 575.98px) {
  .popular-packages-section.full-bleed .container {
    max-width: 100%;
    padding-left: 0.6rem;
    padding-right: 0.6rem;
  }
  .popular-packages-section.full-bleed .row {
    margin-left: 0;
    margin-right: 0;
  }
  .popular-packages-section.full-bleed .col-12 {
    padding-left: 0.6rem;
    padding-right: 0.6rem;
  }
}
